                        ...
                        UserScript(
                          source: """
                            (function() {
                              ...
                              // 3. Notification
                              // Report 'granted' so the site's audio/ringtone logic is
                              // not gated behind a denied-permission check. In a regular
                              // browser the user's real permission passes this check;
                              // in WebView the old 'denied' mock silently blocked the
                              // site from ever calling playSound().
                              // new Notification() is a no-op constructor — Flutter/FCM
                              // already shows tray notifications natively, so we must
                              // prevent the web page from creating duplicate OS alerts.
                              try {
                                function MockNotification(title, options) {
                                  // Intentional no-op: actual tray notifications are
                                  // managed by Flutter, not the web page.
                                }
